...
use ratatui::Frame;
use ratatui::layout::{Alignment, Constraint, Direction, Layout, Margin, Offset, Position, Rect, Size};
use ratatui::style::{Color, Style, Stylize};
use ratatui::symbols::border;
use ratatui::text::{Line, Span, Text};
use ratatui::widgets::{Block, Borders, Cell, Clear, Padding, Paragraph, Row, Scrollbar, ScrollbarOrientation, Table, Wrap};
...
fn compute_board_layouts(area: Rect, square_size: u16) -> (Rc<[Rect]>, Rc<[Rect]>, Rc<[Rect]>) {
    ...
    let board_horizontal = Layout::horizontal(
        [Constraint::Length(3), Constraint::Length(square_size * 8)]
    ).split(area);

    // add additional row for label
    let board_vertical = Layout::vertical(
        [Constraint::Length((square_size/2) * 8), Constraint::Length(1)]
    ...
    let file_label_layout = Layout::horizontal([Constraint::Length(square_size); 8])
...
fn actual_file(file: usize, flipped: bool) -> usize {
    if flipped { 7 - file } else { file } // Flip files
}

fn actual_rank(rank: usize, flipped: bool) -> usize {
    if flipped { rank } else { 7 - rank } // Flip ranks
}

fn render_square(frame: &mut Frame, file_layout: &Rc<[Rect]>, rank: usize, file: usize, flipped: bool) {
...
fn render_piece(frame: &mut Frame, app: &App, file_layout: &Rc<[Rect]>, file: usize, piece: char, flipped: bool) {
    let actual_file = actual_file(file, flipped);
...
fn render_board(app: &App, frame: &mut Frame, area: Rect, large_board: bool) {
    let square_size = if large_board { LARGE_SQUARE_SIZE } else { DEFAULT_SQUARE_SIZE };
    ...
    for (rank, files) in pieces.iter().enumerate().rev() {
        ...
        let file_layout = Layout::horizontal([Constraint::Length(square_size); 8]).split(rank_layout[rank_layout_idx]);
...
fn render_moves(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    *app.table_state.offset_mut() = app.scroll_offset;
    ...
    if app.show_scrollbar {
        frame.render_stateful_widget(scrollbar, moves_layout[1].inner(Margin{horizontal: 0, vertical: 1}), &mut app.scrollbar_state);
        }
}
